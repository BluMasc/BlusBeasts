...
public class SquidSortHelper {
    ...
    private static final String[] END_WORDS = {"end", "void", "chorus", "shulker","dragon","squid"};
    private static final String[] NETHER_WORDS = {"nether", "fire", "lava", "flint", "ghast","blaze","gun","wart","warped","crimson","soul","magma"};
    ...
    private static boolean hasMoonWord(ItemStack stack) {
        ...
        for (String word : END_WORDS) if (name.contains(word)) return true;
        return false;
    }

    private static boolean hasSunWord(ItemStack stack) {
        String name = getItemName(stack).toLowerCase();
        for (String word : NETHER_WORDS) if (name.contains(word)) return true;
        return false;
    }
