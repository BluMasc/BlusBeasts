...
public class EndsquidModel <T extends EndSquidEntity> extends HierarchicalModel<T> {
    // This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(BlusBeastsMod.MODID, "endsquid"), "main");
    ...
    public EndsquidModel(ModelPart root) {
    ...
    @Override
    public void setupAnim(EndSquidEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);

        this.animate(entity.swirlAnimationState, EndsquidAnimation.swirl, ageInTicks, 1f);
        this.animate(entity.idleAnimationState, EndsquidAnimation.idle, ageInTicks, 1f);
        this.animateWalk(EndsquidAnimation.walk, limbSwing, limbSwingAmount, 2.0f, 1.0f);
        this.animate(entity.sitAnimationState, EndsquidAnimation.sitting, ageInTicks, 1f);
    }
    ...
    public void renderOnShoulder(PoseStack poseStack, VertexConsumer buffer, int packedLight, int packedOverlay,
        ...
        this.animate(dummySit, EndsquidAnimation.sitting, ageInTicks, 1f);
